...
function getImageCachePath(imageUrl: string, cacheDir: string): string {
  const ext = imageUrl.match(/\.(png|jpg|jpeg|gif|webp|svg)(\?|$)/iu)?.[1] ?? "png";
...
function saveToDiskCache(data: OgpData, cacheDir: string): void {
  ...
  writeFileSync(getMetaCachePath(data.url, cacheDir), JSON.stringify(data, null, 2));
}

async function downloadImage(imageUrl: string, cacheDir: string): Promise<string | null> {
  const localPath = getImageCachePath(imageUrl, cacheDir);
  if (existsSync(localPath)) {
    return "/" + localPath.replace(/^public\//u, "");
  }
...
async function fetchOgp(url: string, options: Required<Options>): Promise<OgpData | null> {
  // 1. Memory cache
  ...
  try {
    ...
    const title =
      html.match(/property="og:title".*?content="([^"]+)"/u)?.[1] ??
      html.match(/<title>([^<]+)<\/title>/u)?.[1] ??
      new URL(url).hostname;
    const description = html.match(/property="og:description".*?content="([^"]+)"/u)?.[1] ?? "";
    const image = html.match(/property="og:image".*?content="([^"]+)"/u)?.[1] ?? "";
...
function createCardHtml(data: OgpData, options: Required<Options>): string {
  ...
  const inner =
    options.thumbnailPosition === "left" ? `${thumbnail}${content}` : `${content}${thumbnail}`;
...
function isStandaloneUrl(paragraph: Paragraph): string | null {
  ...
  if (child.type === "text") {
    const match = child.value.match(/^https?:\/\/[^\s]+$/u);
