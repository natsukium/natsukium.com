...
describe("Layout OGP meta tags", () => {
  ...
  const renderLayout = (props: Record<string, unknown>, url = `${BASE_URL}/`) => {
  ...
  it("renders og:url meta tag with correct URL", async () => {
    ...
    expect(result).toMatch(
      /property="og:url"[^>]*content="https:\/\/natsukium\.com\/posts\/test"/u,
    );
  });
  ...
  it("renders og:image meta tag with absolute URL", async () => {
    ...
    expect(result).toMatch(/property="og:image"[^>]*content="https:\/\//u);
  });
  ...
  it("renders twitter:image meta tag with name attribute and absolute URL", async () => {
    ...
    expect(result).toMatch(/name="twitter:image"[^>]*content="https:\/\//u);
  });
