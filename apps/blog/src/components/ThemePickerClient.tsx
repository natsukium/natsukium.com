  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      requestAnimationFrame(() => {
        ...
        const currentItem = dropdownRef.current?.querySelector(`[data-theme="${currentTheme}"]`);
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case "ArrowDown":
          event.preventDefault();
          setFocusedIndex((prev) => (prev < sortedThemes.length - 1 ? prev + 1 : 0));
          break;
        case "ArrowUp":
          event.preventDefault();
          setFocusedIndex((prev) => (prev > 0 ? prev - 1 : sortedThemes.length - 1));
          break;
        case "Enter":
          event.preventDefault();
  ...
  return (
    ...
    >
      ...
      >
        {currentThemeData && (
          <div className="flex">
            {/* Palette swatches are fixed-position and never reordered, and colors may
                repeat within a scheme, so the index is the only stable unique key. */}
          ...
          <div className="p-2">
            <input
              ...
              aria-activedescendant={
                focusedIndex >= 0 ? `theme-option-${sortedThemes[focusedIndex]?.slug}` : undefined
              }
            />
          </div>
          <div
              ...
              >
                ...
                >
                  <div className="flex shrink-0 flex-col gap-0.5">
                    {/* Fixed-position palette swatches; colors may repeat, so index is the stable key. */}
                    {/* oxlint-disable react/no-array-index-key */}
                    <div className="flex">
                  ...
                  <span className="ml-auto shrink-0 text-xs text-400">{theme.variant}</span>
                </button>
              </div>
            ))}
          </div>
